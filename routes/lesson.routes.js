...
r.post("/admin/courses/:courseId/levels", requireAuth, requireRole(["ADMIN"]), validate(courseIdParam), validate(levelCreateRules),uploadCourseLevelImage.single('imageUrl'), adminCreateLevel);
r.put("/admin/levels/:id", requireAuth, requireRole(["ADMIN"]), validate(idParam), validate(levelCreateRules),uploadCourseLevelImage.single('imageUrl'), adminUpdateLevel);
